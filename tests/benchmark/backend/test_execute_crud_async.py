"""Asynchronous PostgreSQL direct backend CRUD benchmarks."""
...
def test_postgres_backend_execute_insert_one_async(benchmark, postgres_backend_async_context):
    ...
    inserted_id = benchmark(lambda: run(insert_row()))
    row = run(execute_find_one_async(context, inserted_id))
...
def test_postgres_backend_execute_find_one_async(benchmark, postgres_backend_async_context):
    context, run = postgres_backend_async_context
    record_id = context.record_ids[len(context.record_ids) // 2]
    row = benchmark(lambda: run(execute_find_one_async(context, record_id)))
...
@pytest.mark.benchmark_async
@pytest.mark.benchmark_write
def test_postgres_backend_execute_update_one_async(benchmark, postgres_backend_async_context):
    context, run = postgres_backend_async_context
    counter = {"value": 0}
    ...
    record_id, username = benchmark(lambda: run(update_row()))
    row = run(execute_find_one_async(context, record_id))
...
@pytest.mark.benchmark_async
@pytest.mark.benchmark_write
def test_postgres_backend_execute_delete_one_async(benchmark, postgres_backend_async_context):
    context, run = postgres_backend_async_context
    counter = {"value": 0}
    ...
    record_id = benchmark(lambda: run(delete_row()))
    result = run(
        context.backend.execute(
            context.sql["find_one"],
            (record_id,),
            options=DQL_OPTIONS,
        )
    )
