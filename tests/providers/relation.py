    ...
    def _setup_relation_boundary_sync(self, scenario_name):
        if not self._sync_relation_boundary_setup:
            ...
            self._configure_with_shared_backend(
                BoundaryProfile, config, backend_class, backend,
            )
            self._configure_with_shared_backend(
                BoundaryPost, config, backend_class, backend,
            )
...
class RelationAsyncProvider(RelationProviderBase, IRelationAsyncProvider):
    ...
    async def _setup_employee_department_fixtures_async(self, scenario_name):
        _, config = get_scenario(scenario_name)
        await Employee.configure(config, AsyncPostgresBackend)
        backend = Employee.backend()
        await backend.connect()
        await backend.introspect_and_adapt()
        self._active_async_backends.append(backend)
        await self._execute_script_async(backend, EMPLOYEE_DEPARTMENT_SCHEMA)
        self._configure_with_shared_backend(Department, config, AsyncPostgresBackend, backend)
        return Employee, Department

    async def _setup_author_book_fixtures_async(self, scenario_name):
        _, config = get_scenario(scenario_name)
        await Author.configure(config, AsyncPostgresBackend)
        backend = Author.backend()
        await backend.connect()
        await backend.introspect_and_adapt()
        self._active_async_backends.append(backend)
        await self._execute_script_async(backend, AUTHOR_BOOK_SCHEMA)
        self._configure_with_shared_backend(Book, config, AsyncPostgresBackend, backend)
        self._configure_with_shared_backend(Chapter, config, AsyncPostgresBackend, backend)
        self._configure_with_shared_backend(Profile, config, AsyncPostgresBackend, backend)
        return Author, Book, Chapter, Profile
    ...
    async def _setup_relation_boundary_async(self, scenario_name):
        if not self._async_relation_boundary_setup:
            ...
            backend = self._configure_async_model_without_connection(
                AsyncBoundaryOwner, config,
            )
            self._configure_async_model_without_connection(
                AsyncBoundaryProfile, config, backend,
            )
            self._configure_async_model_without_connection(
                AsyncBoundaryPost, config, backend,
            )
            self._active_async_backends.append(backend)
            await backend.introspect_and_adapt()
    ...
    async def setup_employee_department_fixtures(
        ...
        return await self._setup_employee_department_fixtures_async(scenario_name)

    async def setup_author_book_fixtures(
        self,
        scenario_name: str,
    ) -> Tuple[Type[ActiveRecord], Type[ActiveRecord], Type[ActiveRecord], Type[ActiveRecord]]:
        return await self._setup_author_book_fixtures_async(scenario_name)
