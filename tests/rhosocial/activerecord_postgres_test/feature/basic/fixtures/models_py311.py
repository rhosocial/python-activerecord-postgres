...
"""
Python 3.11+ fixture model definitions.
...
- Exception groups (not used in models)
...
class TypeCase(UUIDMixin, ActiveRecord):
    ...
    def clone(self) -> Self:
        ...
        Uses Python 3.11+ Self type for accurate return type annotation.
        """
...
class User(IntegerPKMixin, TimestampMixin, ActiveRecord):
    ...
    def with_balance(self, new_balance: float) -> Self:
        ...
        Uses Python 3.11+ Self type for fluent interface pattern.
        """
        new_user = self.clone()
    ...
    def activate(self) -> Self:
        """Activate the user and return self.

        Uses Python 3.11+ Self type for chainable method pattern.
        """
        self.is_active = True
        return self

    def deactivate(self) -> Self:
        """Deactivate the user and return self.
...
class ValidatedUser(IntegerPKMixin, ActiveRecord):
    ...
    def validate_record(cls, instance: Self) -> None:
        """Business rule validation using Self type for instance parameter."""
...
class TypeAdapterTest(ActiveRecord):
    ...
    Python 3.11+ version using | syntax.
    """
