...
class PostgresDialect(
    ...
    def format_create_table_statement(self, expr: "CreateTableExpression") -> Tuple[str, tuple]:
        ...
        * Declarative partitioning validation for ``partition`` and legacy
        ...
        2. When `dialect_options` does NOT contain 'like_table':
           - Applies PostgreSQL partition validation when `partition` or legacy
             `partition_by` is present
        ...
        - KEY partitioning is rejected because PostgreSQL does not support it.
        - The preferred API is `partition=PartitionClause(...)`; PartitionClause
          is the standard expression type for partition specification.
        ...
        if getattr(expr, "partition", None) is not None:
            ...
            raw_strategy = strategy.value if hasattr(strategy, "value") else str(strategy)
            normalized = raw_strategy.upper()
            if normalized == "HASH" and not self.supports_hash_table_partitioning():
                raise ValueError("HASH partitioning requires PostgreSQL 11+")
            if normalized == "KEY":
                raise ValueError("PostgreSQL does not support KEY partitioning")
            if not self.supports_partitioned_table_creation():
                raise ValueError("Declarative table partitioning requires PostgreSQL 10+")
