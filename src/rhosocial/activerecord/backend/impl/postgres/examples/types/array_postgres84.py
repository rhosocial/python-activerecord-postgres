"""
PostgreSQL array type operations.
"""
...
create_table = CreateTableExpression(
    ...
    columns=[
        ...
        ColumnDefinition('scores', 'INTEGER[]'),
    ],
)
...
insert_expr = InsertExpression(
    ...
    source=ValuesSource(
        dialect,
        [
            [Literal(dialect, 'Alice'), Literal(dialect, '{85,90,78}')],
            [Literal(dialect, 'Bob'), Literal(dialect, '{92,88,95}')],
            [Literal(dialect, 'Charlie'), Literal(dialect, '{70,75,80}')],
        ],
    ),
)
...
sql_fragment = array_length(dialect, 'scores', 1)
print(f"array_length SQL: {sql_fragment}")

sql_fragment = array_to_string(dialect, 'scores', ', ')
